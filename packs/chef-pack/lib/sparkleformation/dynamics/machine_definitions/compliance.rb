...
SparkleFormation.dynamic(:compliance, :provider => :azure) do |name, opts = {}|
  ...
  parameters do
    set!("#{name}_image_sku".to_sym) do
      type "string"
      default_value registry!(:compliance_default)
      allowed_values registry!(:compliance_versions)
    end
  end
